...
export class Migration1740395204919 implements MigrationInterface {
    name = 'Migration1740395204919'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`payment_method\` (\`id\` int NOT NULL AUTO_INCREMENT, \`dateEntry\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`dateModified\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`additionalNotes\` text NOT NULL, \`enabled\` tinyint NOT NULL DEFAULT 1, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`order\` (\`id\` int NOT NULL AUTO_INCREMENT, \`dateEntry\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`dateModified\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`status\` enum ('PENDING', 'PROCESSING', 'IN TRANSIT', 'DELIVERED') NOT NULL DEFAULT 'PENDING', \`cartId\` int NULL, \`addressId\` int NULL, \`paymentMethodId\` int NULL, UNIQUE INDEX \`REL_fe3963d525b2ee03ba471953a7\` (\`cartId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`address\` ADD CONSTRAINT \`FK_8f7778fdc3992221338f56f2c8b\` FOREIGN KEY (\`userDetailId\`) REFERENCES \`user_detail\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`address\` DROP FOREIGN KEY \`FK_8f7778fdc3992221338f56f2c8b\``);
